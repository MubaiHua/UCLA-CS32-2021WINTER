...
City::City(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), m_histroy(nRows, nCols)
{
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for (int k = 0; k < m_nFlatulans; )
    {
        ...
        if (rowdiff >= -1 && rowdiff <= 1 && coldiff >= -1 && coldiff <= 1) //judge whether is orthogonally or diagonally adjacent
        {
            if (fp->possiblyGetConverted()) { //if preach succeed
                delete m_flatulans[k];
                m_flatulans[k] = m_flatulans[m_nFlatulans - 1];
                m_nFlatulans--;
            }
            else { // if preach failed
                failed = true;
                k++;
            }
        }
